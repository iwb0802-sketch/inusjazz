 ...
 * 사회자 스케줄 현황 페이지
 * /schedule 경로로 접근
 * 수정: 메인 버튼, 민트 프리미엄 디자인, 비활동 제외, 등급순 정렬, 가능 시간대 표시 등
 */
import { useState } from "react";

const API_URL = "/api/schedule";

// 활동 사회자 명단 (비활동 제외: 문학진, 박진영, 김한솔, 강동우, 김민중)
const ALL_EMCEES = [
...
const SLOT_RANGES: Record<string, [number, number]> = {
  am:  [660, 840],   // 11:00~14:00
  pm1: [840, 960],   // 14:00~16:00
  pm2: [960, 1140],  // 16:00~19:00
};
...
function getAvailableSlots(mcName: string, assignedMap: Record<string, number[]>): string[] {
  ...
  Object.entries(SLOT_RANGES).forEach(([key, [start, end]]) => {
    const mid = (start + end) / 2;
    const conflict = times.some(t => Math.abs(t - mid) <= 150);
    if (!conflict) available.push(SLOT_LABELS[key]);
  });
  return available;
}

// 등급순 + 가나다순 정렬
function sortByTierAndName(names: string[]): string[] {
  return [...names].sort((a, b) => {
    const pa = MC_MAP[a] || { tierOrder: 99, name: a };
    const pb = MC_MAP[b] || { tierOrder: 99, name: b };
...
const C = {
  ...
  mint: "#5BB5A2",
  mintLight: "rgba(91,181,162,0.15)",
  mintBorder: "rgba(91,181,162,0.3)",
  gold: "#d4b896",
  goldLight: "rgba(212,184,150,0.15)",
  goldBorder: "rgba(212,184,150,0.3)",
...
function TierBadge({ tier }: { tier: Tier }) {
  const styles: Record<Tier, React.CSSProperties> = {
  ...
  return (
    <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 4, ...styles[tier] }}>
      {tier}
    </span>
  );
}

function McAvatar({ name, size = 44 }: { name: string; size?: number }) {
  const p = MC_MAP[name];
  ...
  if (p?.img && !imgErr) {
    return (
      <img src={p.img} alt={name}
        ...
        style={{ width: size, height: size, borderRadius: "50%", objectFit: "cover", objectPosition: "top", flexShrink: 0, border: `2px solid ${C.mintBorder}` }} />
    );
  }
  return (
    <div style={{ width: size, height: size, borderRadius: "50%", background: `linear-gradient(135deg, ${C.mintLight}, rgba(91,181,162,0.05))`, border: `2px solid ${C.mintBorder}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.38, fontWeight: 700, color: C.mint, flexShrink: 0 }}>
      {name.charAt(0)}
    </div>
  );
}
    ...
    <div style={{ fontFamily: "'Apple SD Gothic Neo','Noto Sans KR',sans-serif", background: C.bg, minHeight: "100vh", color: C.text, padding: "0 0 60px" }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "24px 16px" }}>
        ...
        <div style={{ background: C.card, borderRadius: 16, padding: 20, marginBottom: 24, border: `1px solid ${C.cardBorder}` }}>
          <div style={{ fontSize: 12, color: C.textSub, marginBottom: 10, fontWeight: 600, letterSpacing: 0.5 }}>📅 예식 날짜 선택</div>
          ...
          return (
            <div>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
                {["am","pm1","pm2"].map(key => {
                  const assignedCount = (data.slots[key]||[]).filter((i: any) => i.assigned).length;
                  ...
                  return (
                    <button key={key} onClick={() => setActiveTab(key)}
                      ...
                      {SLOT_LABELS[key]}<br/>
                      <span style={{ fontSize: 10, background: "rgba(255,255,255,0.2)", borderRadius: 10, padding: "1px 6px" }}>{assignedCount}건 배정</span>
                    </button>
                  );
                })}
              </div>
              ...
              {["am","pm1","pm2"].map(key => activeTab !== key ? null : (() => {
                ...
                const sortedAssigned = sortByTierAndName(assignedItems.map((i: any) => i.mc_name))
                  .map(name => assignedItems.find((i: any) => i.mc_name === name))
                  .filter(Boolean);
                ...
                return (
                  <div key={key}>
                    {/* 배정완료 사회자 */}
                    <div style={{ fontSize: 11, fontWeight: 700, color: C.textMuted, letterSpacing: 1, marginBottom: 10, paddingLeft: 4, textTransform: "uppercase" }}>
                      배정된 사회자
                    </div>
                    ...
                    ) : sortedAssigned.map((item: any, i: number) => {
                      const p = MC_MAP[item.mc_name];
                      const otherAvail = getAvailableSlots(item.mc_name, assignedMap).filter(s => s !== SLOT_LABELS[key]);
                      return (
                        <div key={i} style={{ background: C.red, border: `1px solid ${C.redBorder}`, borderRadius: 14, padding: "14px 16px", marginBottom: 10, display: "flex", alignItems: "center", gap: 14 }}>
                          <a href={p?.url || "https://www.inusmc.co.kr/#mc"} target="_blank" rel="noopener noreferrer" style={{ flexShrink: 0 }}>
                            <McAvatar name={item.mc_name} size={48} />
                          </a>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                              <a href={p?.url || "https://www.inusmc.co.kr/#mc"} target="_blank" rel="noopener noreferrer"
                                style={{ fontSize: 15, fontWeight: 700, color: C.text, textDecoration: "none" }}>
                                {item.mc_name}
                              </a>
                      ...
                      {availableMcs.length === 0 ? (
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
                          {availableMcs.map(name => {
                            ...
                            const [imgErr, setImgErr] = useState(false);
                            return (
                              <a key={name} href={p?.url || "https://www.inusmc.co.kr/#mc"} target="_blank" rel="noopener noreferrer"
                                style={{ background: C.card, border: `1px solid ${C.cardBorder}`, borderRadius: 12, overflow: "hidden", textDecoration: "none", display: "block", transition: "border-color 0.2s" }}>
                                {p?.img && !imgErr ? (
                                  <img src={p.img} alt={name}
                                    onError={() => setImgErr(true)}
