...
2. [답변지 반영 방식 - 매우 중요] 신랑신부 답변지의 문장을 절대 그대로 옮겨 붙이지 마세요. 답변에 담긴 사실과 감정은 그대로 유지하되, 반드시 사회자가 하객 앞에서 소개하는 3인칭 존댓말 화법으로 다시 써서 넣고, 그 다듬은 문장을 <answer>...</answer>로 감싸세요. 이 표시는 엑셀에서 빨간색 굵게 처리됩니다.
2-1. 답변자의 1인칭 말투("~했어요", "~빠졌습니다", "저는", "제가", "우리 신랑")를 script에 그대로 남기지 마세요. 사회자가 전하는 문장("신부님께서는 ~라고 이야기해 주셨습니다", "~한 점에 마음이 끌리셨다고 합니다")으로 바꿔 쓰세요.
2-2. 답변이 나열식이거나 구어체·비문이면 사회자가 낭독하기 좋은 한두 문장으로 정돈하세요. 문장을 다듬는 것은 허용되지만 새로운 사실을 더하거나 의미를 바꾸는 것은 금지입니다.
2-3. 예시 - 답변 원문: "제일 먼저 귀여운 외모에 호감을 느꼈어요. 그리고 외모와는 반전되는 든든한 성격에 빠졌습니다." → script: "신부님께서는 <answer>처음에는 신랑님의 귀여운 인상에 마음이 갔지만, 그 인상과는 다른 듬직하고 든든한 모습에 더 깊이 반하게 되셨다고</answer> 이야기해 주셨습니다."
2-4. 예시 - 답변 원문: "귀여운 인디언 보조개와 맑고 깨끗한 피부가 매력입니다." → script: "신랑님께서는 <answer>신부님의 귀여운 인디언 보조개와 맑고 깨끗한 피부</answer>를 가장 큰 매력으로 꼽아 주셨습니다."
2-5. [누락 금지 - 매우 중요] 문장을 다듬는 과정에서 답변에 적힌 매력 포인트·키워드·항목을 하나라도 빼지 마세요. 답변자가 A, B, C 세 가지를 적었으면 다듬은 문장에도 A, B, C가 모두 살아 있어야 합니다. 위 2-4 예시에서 "보조개"와 "맑고 깨끗한 피부"는 둘 다 반드시 남아야 하며, 하나로 뭉뚱그리거나 "맑은 미소" 같은 다른 표현으로 대체해서는 안 됩니다.
2-6. 다듬기는 어순·연결어·어미·존댓말을 사회자 화법으로 바꾸는 수준까지만 허용합니다. 답변에 등장한 구체적 명사(외모 특징, 성격 표현, 상황, 이유 등)는 원문 단어를 그대로 살리거나 뜻이 완전히 같은 표현만 쓰세요. 요약·축약·의역으로 정보량을 줄이는 것은 금지입니다.
2-7. 답변이 길더라도 임의로 잘라내지 말고, 문장을 나누어서라도 내용 전부를 사회자 화법으로 전달하세요. 길이가 문제라면 note에 "낭독 시간 조절 필요"를 적으세요.
2-8. 답변 문장을 그대로 인용해야 의미가 살아나는 짧은 표현(별명, 애칭, 한 줄 다짐 등)은 예외적으로 원문을 유지할 수 있습니다.
