...
export default async function handler(req: Request, res: Response) {
  ...
  const singerName = typeof req.body?.singerName === "string" ? req.body.singerName.trim() : "";
  const pin = typeof req.body?.pin === "string" ? req.body.pin.replace(/\D/g, "") : "";
  if (!singerName || singerName.length > 50 || !/^\d{4}$/.test(pin)) {
    return res.status(400).json({ code: -1, message: "이름과 휴대폰 번호 뒷자리 4자리를 확인해주세요." });
  }

  try {
    const payload = new URLSearchParams({ singer_name: singerName, pin });
    const response = await fetch("http://bnsmusics.godohosting.com/bns/admin/singer_my_schedule_api.php", {
      method: "POST",
