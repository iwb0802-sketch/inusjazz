...
// 활동 싱어용 본인 확인 목록입니다. 전화번호 뒷자리는 서버에서 검증합니다.
const SINGER_NAMES = [
  "강윤재", "권소이", "김대희", "김영일", "김은서", "김지유", "박달해",
  "서동준", "손우경", "안우석", "이윤주", "임세라", "제은빈", "최병준",
];
...
export default function SingerMySchedule() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setResult(null);
    try {
      const response = await fetch("/api/singer-my-schedule", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ singerName, pin }),
      });
          ...
          <section className="mx-auto max-w-xl overflow-hidden rounded-[28px] border border-[#65d4c0]/30 bg-white/[0.04] shadow-2xl shadow-black/20">
            <div className="border-b border-[#65d4c0]/25 bg-[#0d2938] px-6 py-7 sm:px-8">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-xs font-bold tracking-[0.16em] text-[#70ddca]">MY SINGER SCHEDULE</p>
            ...
            <form onSubmit={submit} className="space-y-5 px-6 py-7 sm:px-8 sm:py-8">
              <div ref={nameMenuAnchorRef}>
                <span className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-100"><UserRound size={16} className="text-[#74dfcc]" /> 싱어 이름</span>
                <button type="button" onClick={toggleNameMenu} aria-haspopup="listbox" aria-expanded={isNameMenuOpen} className="flex h-14 w-full items-center justify-between rounded-xl border border-white/15 bg-[#10283a] px-4 text-left text-base font-semibold text-white outline-none transition hover:border-[#70ddca]/65 focus:border-[#70ddca] focus:ring-2 focus:ring-[#70ddca]/25">
                  <span className={singerName ? "text-white" : "text-slate-300"}>{singerName || "이름을 선택해주세요"}</span>
              ...
              {isNameMenuOpen && (
                <div ref={nameMenuPanelRef} role="listbox" style={{ left: nameMenuPosition.left, top: nameMenuPosition.top, width: nameMenuPosition.width, maxHeight: nameMenuPosition.maxHeight || "calc(100dvh - 24px)", WebkitOverflowScrolling: "touch", overscrollBehavior: "contain", paddingBottom: "max(0.375rem, env(safe-area-inset-bottom))" }} className="fixed z-[100] grid grid-cols-2 gap-1.5 overflow-y-scroll rounded-xl border border-[#75dac9]/70 bg-[#14354a] p-1.5 shadow-2xl shadow-black/70 sm:grid-cols-3">
                  {SINGER_NAMES.map((name) => (
                    <button key={name} type="button" role="option" aria-selected={singerName === name} onClick={() => { setSingerName(name); setIsNameMenuOpen(false); setError(""); }} className={`min-h-[42px] w-full rounded-lg px-3 py-2 text-left text-[15px] font-bold leading-5 transition ${singerName === name ? "bg-[#55cdb8] text-[#07202a]" : "text-white hover:bg-white/15 hover:text-[#a4f1e3]"}`}>
                      {name}
                    </button>
                  ))}
                </div>
              ...
              <p className="flex items-start gap-2 text-xs leading-5 text-slate-400"><LockKeyhole size={14} className="mt-0.5 shrink-0 text-[#6dccbd]" />싱어는 등록된 휴대폰 번호 뒷자리로 확인합니다. 관리자는 통합 비밀번호로 어느 싱어 일정이든 조회할 수 있습니다. 고객 성명과 연락처는 표시되지 않습니다.</p>
            </form>
          </section>
        ) : (
          <section>
            <div className="mb-7 flex flex-col gap-5 border-b border-white/15 pb-6 xl:flex-row xl:items-end xl:justify-between">
              <div>
                <p className="text-xs font-bold tracking-[0.16em] text-[#70ddca]">MY SINGER SCHEDULE</p>
                <h2 className="mt-2 text-3xl font-extrabold tracking-tight"><span className="text-[#7ce4d2]">{result.singer_name}</span> 싱어님의 예정 일정</h2>
