...
| `BLOB_READ_WRITE_TOKEN` | Vercel Blob 직접 업로드 토큰 |

Vercel Storage에서 **Public Blob store**를 연결해야 Replicate가 업로드 음원을 읽을 수 있습니다. 원본은 `audio-source/` 경로에 저장되며, 운영 전 Vercel Blob의 수명 주기 정책으로 자동 삭제 기간을 설정하는 것을 권장합니다.
