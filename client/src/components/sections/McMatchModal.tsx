...
function calcRecommendations(answers: { id: string; value: string }[]): { mcs: typeof MCS; reason: string } {
  ...
  // 예식장 점수
  // 1순위: 호텔→최윤아, 고급홀→장윤태, 일반홀→이도영, 야외→길상우
  const venueMap: Record<string, string[]> = {
    hotel:        ["최윤아", "이우영", "장윤태", "고승범", "김민수"],
    premium_hall: ["장윤태", "김선혁", "이우영", "최윤아", "석재선"],
    normal_hall:  ["이도영", "고승범", "길상우", "김민수", "석재선"],
    outdoor:      ["길상우", "이도영", "고승범", "김민수", "석재선"],
  };
  venueMap[venue]?.forEach((name, i) => { scores[name] += 5 - i; });

  // 하객수 점수
  // 1순위: 소규모→길상우, 중간→이도영, 대규모→이우영
  const guestsMap: Record<string, string[]> = {
    small:  ["길상우", "고승범", "김민수", "석재선", "이도영"],
    medium: ["이도영", "김민수", "석재선", "장윤태", "고승범"],
    large:  ["이우영", "최윤아", "김선혁", "장윤태", "이도영"],
  };
  guestsMap[guests]?.forEach((name, i) => { scores[name] += 5 - i; });

  // 분위기 점수
  // 1순위: 품격→최윤아, 감동→석재선, 유쾌→길상우
  const moodMap: Record<string, string[]> = {
    formal:    ["최윤아", "이우영", "장윤태", "김선혁", "고승범"],
    emotional: ["석재선", "이도영", "장윤태", "고승범", "최윤아"],
    joyful:    ["길상우", "김선혁", "이도영", "김민수", "고승범"],
  };
  moodMap[mood]?.forEach((name, i) => { scores[name] += 5 - i; });

  // 예산 점수
  // 1순위: 합리적→고승범, 중간→김선혁, 프리미엄→최윤아
  const budgetMap: Record<string, string[]> = {
    budget:  ["고승범", "이도영", "길상우", "김민수", "석재선"],
    mid:     ["김선혁", "석재선", "이도영", "장윤태", "고승범"],
    premium: ["최윤아", "이우영", "장윤태", "김선혁", "김민수"],
  };
  budgetMap[budget]?.forEach((name, i) => { scores[name] += 5 - i; });

  // 우선순위 점수
  // 1순위: 재미→길상우, 감동→이도영, 깔끔→김민수
  const priorityMap: Record<string, string[]> = {
    fun:     ["길상우", "김선혁", "이도영", "고승범", "김민수"],
    emotion: ["이도영", "석재선", "장윤태", "최윤아", "고승범"],
    clean:   ["김민수", "최윤아", "이우영", "고승범", "김선혁"],
  };
