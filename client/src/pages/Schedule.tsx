...
// 운영 DB에 기존 이름(공백준) 또는 현재 공개 이름(고명준)으로 저장된 배정을
// 하나의 공개 프로필과 예약 가능 여부로 통합한다.
const DB_EMCEE_NAME_ALIASES: Record<string, string> = {
  "공백준": "고명준",
  "고명준": "고명준",
};
function resolvePublicEmceeName(name?: string): string {
  const cleaned = (name || "").trim();
  return DB_EMCEE_NAME_ALIASES[cleaned] || cleaned;
}
function isPublicScheduleItem(item: any): boolean {
  const publicName = resolvePublicEmceeName(item?.mc_name);
...
const MC_PROFILES: McProfile[] = [
  ...
  { name:"고명준",  tier:"PREMIUM",  tierOrder:1, img:"https://files.manuscdn.com/user_upload_by_module/session_file/310519663604364385/cbMnodzCSYqHlMtn.webp", url:KAKAO_URL, desc:"웨딩 전문 사회자", audio:"https://files.manuscdn.com/user_upload_by_module/session_file/310519663604364385/ZObjnFnQqOmaHGvg.mp3", imgPos:"50% 15%" },
  ...
  { name:"이상운",  tier:"BEST",     tierOrder:2, img:"https://files.manuscdn.com/user_upload_by_module/session_file/310519663604364385/wgoNWOqnhecIESbf.webp", url:KAKAO_URL, desc:"웨딩 전문 사회자", audio:"https://files.manuscdn.com/user_upload_by_module/session_file/310519663604364385/kgSWJrpcDmugppgm.mp3", imgPos:"50% 23%" },
...
function getAssignedMap(slots: Record<string, any[]>) {
  ...
  ["am","pm1","pm2","other"].forEach(k => {
    (slots[k]||[]).forEach((item: any) => {
      const publicName = resolvePublicEmceeName(item.mc_name);
      if (isPublicScheduleItem(item)) {
        if (!map[publicName]) map[publicName] = [];
        map[publicName].push(parseTimeToMin(item.time));
      }
    });
  });
  return map;
    ...
    <div style={{ fontFamily:"'Apple SD Gothic Neo','Noto Sans KR',sans-serif", background:C.bg, minHeight:"100vh", color:C.text, padding:"0 0 100px", boxSizing:"border-box", overflowX:"hidden" }}>
      ...
      <div style={{ maxWidth:640, margin:"0 auto", padding:"18px 16px 24px" }}>
        ...
        {data && (() => {
          ...
          const hasOther = (data.slots["other"]||[]).filter(isPublicScheduleItem).length > 0;
          ...
          return (
            <div>
              ...
              <div style={{ display:"flex", gap:6, marginBottom:12, flexWrap:"nowrap", overflowX:"auto" }}>
                {tabs.map(key => {
                  const cnt = (data.slots[key]||[]).filter(isPublicScheduleItem).length;
              ...
              {tabs.map(key => {
                ...
                const assignedItems = (data.slots[key]||[])
                  .filter(isPublicScheduleItem)
                  .map((i: any) => ({ ...i, mc_name: resolvePublicEmceeName(i.mc_name) }));
